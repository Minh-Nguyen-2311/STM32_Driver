...
void Get_Time(TIME *tim)
{
	...
	tim->seconds = bcdToDec(get_time[0]);
	tim->minutes = bcdToDec(get_time[1]);
	tim->hour = bcdToDec(get_time[2]);
	tim->dayofweek = bcdToDec(get_time[3]);
	tim->dayofmonth = bcdToDec(get_time[4]);
	tim->month = bcdToDec(get_time[5]);
	tim->year = bcdToDec(get_time[6]);
}
